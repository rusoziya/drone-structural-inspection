...
#
# NEW FUNCTIONALITIES (from Code 1):
#  • B‑Spline smoothing of the A* path.
#  • Conversion of the smooth path to a ROS Path message.
#  • Trajectory generation with yaw using the TrajectoryGenerationModule.
#
# Assumptions:
...
#
# 5. An A* algorithm is used to compute a continuous 3D path, which is then used directly.
#
...
# NEW IMPORTS for smoothing (from Code 1)
...
# ------------------------
# New Helper Functions from Code 1
# ------------------------
def create_path_msg(points, frame_id="earth"):
    """
    Create a ROS Path message from a list of 3D points.
    (Requires nav_msgs.msg.Path and geometry_msgs.msg.PoseStamped.)
    """
...
def smooth_path_bspline(path, degree=3, smoothing=0):
    """
    Smooth a path using a B‑Spline interpolation.
    If the number of points is less than degree+1, the original path is returned.
    """
    import numpy as np
    ...
    # Determine an appropriate number of points for the smooth path
    num_points = max(len(points), int(8 + 8 * np.mean(dists)))
    ...
    # Ensure endpoints remain the same
    smoothed_path[0] = path[0]
    smoothed_path[-1] = path[-1]
    return smoothed_path

# ------------------------
# Existing Helper Functions (from Code 2)
# ------------------------
...
def plan_astar(start, goal, obstacles, bounds, resolution, planning_time_limit=PLANNING_TIME_LIMIT):
    ...
    with the given resolution (in meters).
...
# --------------------------
def plan_segment(start, goal, obstacles, bounds, astar_resolution, planning_time_limit, recursion_depth=MAX_RECURSION_DEPTH):
    """
    Recursively plan a collision-free path between 'start' and 'goal' using A*,
    subdividing the path if collisions are found along any segment.
    Returns the path (list of [x,y,z]) and the accumulated planning time.
    """
    path, ptime = plan_astar(start, goal, obstacles, bounds,
    ...
    # If the A* path itself is collision-free, we are good
    if is_path_collision_free(path, obstacles):
        return path, ptime
    else:
        # We have a path but it is NOT fully collision-free when checked with finer resolution.
        # Subdivide around the arithmetic midpoint to try smaller steps.
        ...
        print(f"Subdividing segment using valid arithmetic midpoint: {mid_point}")
        ...
        if path1 is None or path2 is None:
            # If either sub-path fails, the segment fails
            return None, ptime + (time1 or 0) + (time2 or 0)

        # -------------------------
        # FIX FOR DISCONTINUITY:
        # Snap path2[0] to path1[-1] if they are close, then merge
        # -------------------------
        import numpy as np
        combined = path1
        if np.allclose(path1[-1], path2[0], atol=1e-6):
            combined += path2[1:]
        else:
            combined += path2
...
# --------------------------
# Plotting Functions (unchanged from Code 2)
# --------------------------
...
def plot_paths(planned_paths, waypoints, obstacles=None, markers_list=None):
    ...
    ax3d.scatter(wp_x, wp_y, [p[2] for p in waypoints], c='r', marker='o', label="Waypoints")
    for i, (x, y, z) in enumerate(zip(wp_x, wp_y, [p[2] for p in waypoints])):
...
class SingleNodeDroneMission(DroneInterface):
    def __init__(self, drone_id, use_sim_time=True, verbose=False):
        ...
        # NEW: Attach the trajectory generation module
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Run the mission:
      ...
      - The drone follows the computed (and B‑Spline smoothed) path via trajectory generation
        with continuous yaw.
    ...
    for i in range(len(ordered_points) - 1):
        ...
        path, planning_time = plan_segment(seg_start, seg_goal, obstacles, bounds, fixed_astar_resolution, PLANNING_TIME_LIMIT)
        if path is None:
        ...
        # NEW: Smooth the planned path using B‑Spline smoothing
        smoothed_path = smooth_path_bspline(path)
        planned_paths.append(smoothed_path)
        seg_length = sum(compute_euclidean_distance(smoothed_path[j], smoothed_path[j+1]) for j in range(len(smoothed_path)-1))
        total_path_length += seg_length
        segment_planning_times.append(planning_time)
        segment_lengths.append(seg_length)
        # NEW: Follow the entire smoothed path using trajectory generation with yaw
    ...
    metrics = {
        ...
        "total_mission_time": mission_duration
    }
