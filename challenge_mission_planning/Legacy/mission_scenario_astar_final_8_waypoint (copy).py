...
def plan_segment(start, goal, obstacles, bounds, astar_resolution, planning_time_limit, recursion_depth=MAX_RECURSION_DEPTH):
    path, ptime = plan_astar(start, goal, obstacles, bounds, resolution=astar_resolution, planning_time_limit=planning_time_limit)
    if path is None:
        return None, ptime
    ...
    if is_path_collision_free(path, obstacles):
        ...
        path1, time1 = plan_segment(start, mid_point, obstacles, bounds, astar_resolution, planning_time_limit, recursion_depth - 1)
        path2, time2 = plan_segment(mid_point, goal, obstacles, bounds, astar_resolution, planning_time_limit, recursion_depth - 1)
