...
SAFETY_MARGIN = 0.4        # Additional margin (in meters) added around each obstacle 
COLLISION_CHECK_RESOLUTION = 0.2  # Step size for interpolation (in meters)
MAX_RECURSION_DEPTH = 100000     # Maximum subdivisions if in collision

PLANNING_TIME_LIMIT = 25.0         # Time limit for planning each segment (s)
PLANNER_RANGE_SMALL = 1.0          # Fixed planner range for segments

# ------------------------
# Global Metrics and Data Logging Variables
# ------------------------
fallback_count = 0
segment_planning_times = []  # planning time per segment
segment_lengths = []         # path length per segment
global_rrt_tree_data = []    # store RRT tree data for each segment
...
def is_state_valid_cuboids(state, obstacles):
    ...
    for obs in obstacles:
        ...
        hx, hy, hz = dx/2.0 + SAFETY_MARGIN, dy/2.0 + SAFETY_MARGIN, dz/2.0 + SAFETY_MARGIN
        if (ox-hx <= x <= ox+hx and oy-hy <= y <= oy+hy and oz-hz <= z <= oz+hz):
            return False
    return True
...
def get_expected_marker_id_for_viewpoint(viewpoint, markers_list):
    ...
    for marker in markers_list:
        ...
        if dist < best_dist:
            best_dist, best_id = dist, marker["marker_id"]
    return best_id
...
def apply_3opt_move(tour, i, j, k, distance_matrix):
    ...
    best_cost = sum(distance_matrix[best_candidate[x]][best_candidate[x+1]] for x in range(len(best_candidate)-1))
    for candidate in candidates[1:]:
        cost = sum(distance_matrix[candidate[x]][candidate[x+1]] for x in range(len(candidate)-1))
        if cost < best_cost:
...
def solve_tsp_3opt(distance_matrix, max_iterations=1000):
    ...
    while remaining:
        ...
        current = next_city
    def tour_length(t):
        return sum(distance_matrix[t[x]][t[x+1]] for x in range(len(t)-1))
    best_tour = tour[:]
...
# --------------------------
# Modified plan_segment with midpoint perturbation if invalid
# --------------------------
def plan_segment(start, goal, obstacles, bounds, planner_range, planning_time_limit, recursion_depth=MAX_RECURSION_DEPTH):
    ...
    if is_path_collision_free(path, obstacles):
        ...
        if not is_state_valid_cuboids(mid_point, obstacles):
            # Try up to 5 perturbations to find a valid midpoint
            found_valid = False
            for attempt in range(5):
                perturbation = [random.uniform(-0.5, 0.5) for _ in range(3)]
                candidate = [mid_point[i] + perturbation[i] for i in range(3)]
                if is_state_valid_cuboids(candidate, obstacles):
                    mid_point = candidate
                    found_valid = True
                    print(f"Arithmetic midpoint perturbed successfully on attempt {attempt+1}: {mid_point}")
                    break
            if not found_valid:
                print("Arithmetic midpoint is invalid even after perturbations; aborting segment.")
                return None, ptime
...
# --------------------------
# Plotting Functions (saving figures as PNG in script directory)
# --------------------------
...
def plot_paths(planned_paths, waypoints, ground_truth, obstacles=None, markers_list=None):
    ...
    legend_2d = ax2d.legend(fontsize=10)
    legend_2d.get_frame().set_alpha(0.6)
    plt.tight_layout()
    file_path_2d = os.path.join(script_dir, "2D_Trajectory.png")
    fig2d.savefig(file_path_2d, dpi=300)
    print("Saved 2D_Trajectory.png")
    plt.show()
...
def plot_costs(planning_times, path_lengths):
    ...
    fig.savefig(file_path_costs, dpi=300)
    print("Saved Segment_Costs.png")
    plt.show()
...
def plot_rrt_tree(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    for tree in rrt_tree_data:
        ...
        if nodes:
            ...
            if not plotted_rrt_nodes:
                ax.scatter(xs, ys, c='blue', s=10, label="RRT Nodes")
                plotted_rrt_nodes = True
            else:
    ...
    fig.savefig(file_path_rrt, dpi=300)
    print("Saved RRT_Tree.png")
    plt.show()
...
def plot_rrt_tree_3d(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    ax3d.set_ylabel("Y (m)", fontsize=14)
    ax3d.set_zlabel("Z (m)", fontsize=14)
    legend = ax3d.legend(fontsize=10)
    legend.get_frame().set_alpha(0.6)
    plt.tight_layout()
    file_path_rrt_3d = os.path.join(script_dir, "RRT_Tree_3D.png")
    fig.savefig(file_path_rrt_3d, dpi=300)
    print("Saved RRT_Tree_3D.png")
    plt.show()
...
# --------------------------
# Drone Mission Functions (with return-to-start and landing)
# --------------------------
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    print("Run mission with RRT‑Connect planning, TSP (3‑opt local search) optimization, obstacle avoidance, and marker verification")
    ...
    print(f"TSP order (3‑opt): {permutation}, total straight-line distance: {tsp_distance:.2f} m")
    ...
    total_gt_length = sum(compute_euclidean_distance(ground_truth[i], ground_truth[i+1]) for i in range(len(ground_truth)-1))
    print(f"Total ground truth path length: {total_gt_length:.2f} m")
    ...
    for i in range(len(ordered_points) - 1):
        ...
        for j, point in enumerate(path):
            ...
            yaw_command = interpolate_angle(
                dest_yaw if j==N-1 else math.atan2(path[1][1]-path[0][1], path[1][0]-path[0][0]),
                dest_yaw, t)
    ...
    # Plan return path to starting position
...
if __name__ == '__main__':
    ...
    # Display and save graphs after landing
