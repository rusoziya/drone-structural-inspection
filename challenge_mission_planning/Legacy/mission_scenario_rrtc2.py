...
#
# In addition, it integrates (but currently disables) ArUco marker detection
# for demonstration purposes. The drone will visit all waypoints without verifying
# markers, while still planning around obstacles.
#
# Assumptions:
...
#    margin is added around each obstacle to ensure the drone does not fly too close.
# ------------------------------------------------------------------------------
...
# Safety margin (meters) around each obstacle
SAFETY_MARGIN = 0.2
...
def is_state_valid_cuboids(state, obstacles):
    ...
    for obs in obstacles:
        # Each obstacle in scenario: center (obs["x"], obs["y"], obs["z"])
        # dimensions (obs["d"], obs["w"], obs["h"])
        ...
        # Check if (x, y, z) is inside or too close to the cuboid
        if (ox - hx <= x <= ox + hx and
            oy - hy <= y <= oy + hy and
            oz - hz <= z <= oz + hz):
            # This means the state is in collision
            return False
    # If no collision with any obstacle, it's valid
    return True
...
def plan_rrtconnect(start, goal, obstacles, bounds, planner_range=1.0, planning_time_limit=5.0):
    ...
    Obstacles are provided, and we now perform collision checking with is_state_valid_cuboids.
    """
    ...
    # Define a closure that captures 'obstacles' so we can check collisions
...
class SingleNodeDroneMission(DroneInterface):
    ...
    def __init__(self, drone_id, use_sim_time=True, verbose=False):
        ...
        # Initialize OpenCV ArUco detection (currently disabled in mission logic)
        self.br = CvBridge()
    ...
    def image_callback(self, msg):
        ...
        ArUco detection is performed here, but we are ignoring it in the mission logic.
        """
        try:
        ...
        # We detect markers, but do not use them in the mission logic
    ...
    def wait_for_marker(self, timeout=2.0):
        """
        This function is never called in the mission now, but remains for interface completeness.
        """
        start_time = time.time()
...
def verify_marker_and_adjust(drone_interface: SingleNodeDroneMission,
                             ...
                             timeout=2.0):
    """
    Marker verification is disabled. Always return True so the mission continues.
    """
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict) -> bool:
    ...
    Run the mission:
      ...
      - For each segment between ordered points, plan a path using OMPL RRT‑Connect,
        smooth it, and continuously command the drone along it.
      - Yaw is interpolated along the path so the drone faces the direction of travel.
      - Marker verification is disabled; the drone simply visits all waypoints.
      - Collision checking is enabled, so the planner should avoid obstacles.
    """
    ...
    # Define a mapping from viewpoint IDs (1-10) to the actual ArUco marker IDs (not used now).
    marker_mapping = {
        "1": 24,
        "2": 34,
        "3": 44,
    ...
    for i in range(len(ordered_points) - 1):
        ...
        path, planning_time = plan_rrtconnect(
        ...
        # Marker verification is disabled, so we just call the function which returns True
        current_point = smoothed[-1]
        expected_marker_id = ordered_expected_marker_ids[i+1]
        _ = verify_marker_and_adjust(
            drone_interface,
