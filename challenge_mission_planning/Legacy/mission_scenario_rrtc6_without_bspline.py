...
import rclpy
import random
import os  # <-- Added if you want to build a robust path
...
def load_world_yaml(world_file=None):
    ...
    if world_file is None:
        # Option A: Use the script directory as the base
...
def get_marker_info(world_data):
    ...
    Returns a dictionary mapping the expected marker ID (int) to its info.
    """
    markers = {}
    for obj in world_data.get("objects", []):
        mt = obj.get("model_type", "")
        if mt.startswith("aruco_id") and "_marker" in mt:
            marker_num_str = mt[len("aruco_id"):mt.index("_marker")]
            try:
...
def verify_marker_and_adjust(drone_interface: SingleNodeDroneMission,
    ...
    """
    if expected_marker_id not in marker_info:
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict) -> bool:
    ...
    """
    print("Run mission with RRT‑Connect planning, TSP ordering, continuous commands, obstacle avoidance, and marker verification")
    ...
    # Marker ID mapping from scenario viewpoint keys to actual marker IDs
    marker_mapping = {
    ...
    # Change the path to a relative one:
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # Determine yaw along the path
        if len(planned_path) >= 2:
            start_yaw = math.atan2(planned_path[1][1] - planned_path[0][1],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Single drone mission with RRT‑Connect planning, TSP optimization, obstacle avoidance, and marker verification enabled'
    )
