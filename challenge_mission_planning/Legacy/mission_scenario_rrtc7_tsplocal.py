...
def get_expected_marker_id_for_viewpoint(viewpoint, markers_list):
    """
    For a given viewpoint (with keys x, y, z), find the marker in markers_list whose
    position is closest to the viewpoint and return its marker_id.
    """
...
def apply_3opt_move(tour, i, j, k, distance_matrix):
    """
    For a given tour and indices i, j, k, evaluate possible 3‑opt moves and
    return the best tour among the candidates.
    """
    A = tour[:i]
    B = tour[i:j]
    C = tour[j:k]
    ...
    # 5. Swap and reverse B
    candidates.append(A + C + B[::-1] + D)
    # 6. Swap and reverse C
    candidates.append(A + C[::-1] + B + D)
    # 7. Swap and reverse both B and C
...
def solve_tsp_3opt(distance_matrix, max_iterations=1000):
    """
    Solve the TSP using a local search 3‑opt algorithm.
    Returns the permutation of indices and the total tour length.
    """
    n = len(distance_matrix)
    # initial solution: simple greedy nearest neighbor starting from 0
    tour = [0]
...
class SingleNodeDroneMission(DroneInterface):
    ...
    def wait_for_expected_marker(self, expected_marker_id, timeout=1000.0):
        """
        Waits for the expected marker to be detected.
        Only returns when a marker is detected that exactly matches the expected_marker_id.
        Provides debug outputs for each detection attempt.
        """
        start_time = time.time()
        while time.time() - start_time < timeout:
            # Even though a spinner is running in a separate thread, we can use a short sleep here.
            time.sleep(0.1)
    ...
    def wait_for_marker(self, timeout=1000.0):
        """
        (Legacy) Wait for any marker to be detected.
        """
        start_time = time.time()
        while time.time() - start_time < timeout:
            time.sleep(0.1)
...
# --------------------------
# Marker Verification (Re-enabled)
# --------------------------
def verify_marker_and_adjust(drone_interface: SingleNodeDroneMission,
    ...
    After successful verification, the stored detected marker is reset.
    """
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict) -> bool:
    ...
    # Load world.yaml and get marker information as a list, and a set of marker IDs
    ...
    # Process viewpoint poses from scenario and compute expected marker IDs by proximity.
    ...
    for key in sorted(viewpoint_dict.keys(), key=lambda x: int(x)):
        ...
        expected_marker_ids.append(get_expected_marker_id_for_viewpoint(vp, markers_list))
    ...
    # Solve TSP for the ordering using local search 3‑opt
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # Execute the planned path using a threshold to skip redundant moves.
        current_state = planned_path[0]
        N = len(planned_path)
        for j, point in enumerate(planned_path):
            # Skip this command if the drone is already very near the point.
        ...
        # Wait briefly for the drone to settle before marker detection.
        time.sleep(1.0)
