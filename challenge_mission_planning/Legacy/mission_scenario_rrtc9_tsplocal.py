...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict) -> bool:
    ...
    Run the mission:
      ...
      - ***Treat ArUco markers as bigger obstacles*** (1.0x1.0x1.0).
      - Solve the TSP for optimal visitation using a local search 3‑opt solver.
      - For each segment between ordered points, plan a collision-free path.
    ...
    # ----------------------------------------------------------------
    # Turn each marker into a larger bounding box: 1.0 x 1.0 x 1.0
    # ----------------------------------------------------------------
    for marker in markers_list:
        marker_obstacle = {
            "x": marker["xyz"][0],
            "y": marker["xyz"][1],
            "z": marker["xyz"][2],
            "d": 1.0,  # 1 meter in x dimension
            "w": 1.0,  # 1 meter in y dimension
            "h": 1.0   # 1 meter in z dimension
        }
        obstacles.append(marker_obstacle)

    # Gather viewpoint poses
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # If yaw is None, compute default
        ...
        seg_length = sum(
            compute_euclidean_distance(planned_path[j], planned_path[j+1]) 
        ...
        for j, point in enumerate(planned_path):
            ...
            yaw_command = interpolate_angle(
                dest_yaw if j == N - 1 else math.atan2(
                    planned_path[1][1] - planned_path[0][1],
