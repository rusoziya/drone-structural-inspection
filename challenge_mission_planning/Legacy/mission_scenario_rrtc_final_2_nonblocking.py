...
# returned by RRT‑Connect. The smoothing is applied only between the two RRT‑generated
# waypoints, inserting extra intermediate waypoints when the curvature is high.
# The drone flies through these intermediate waypoints at nominal speed.
#
# Assumptions:
...
#    which is then used directly – except now we apply B‑Spline smoothing between the
#    RRT‑generated endpoints.
#
...
from scipy.interpolate import splprep, splev  # For B-Spline smoothing
...
def smooth_path_bspline(path, degree=3, smoothing=0):
    """
    Smooth a given path using B-Spline interpolation.
    This function generates intermediate waypoints based on the curvature of the path.
    The endpoints (RRT‑generated waypoints) are kept fixed.
    The number of intermediate points is chosen adaptively based on the average curvature.
    """
    points = np.array(path)
    if len(points) < degree + 1:
        return path  # Not enough points to fit a spline; return the original path.

    # Compute cumulative distances to serve as the parameter t.
    dists = np.sqrt(np.sum(np.diff(points, axis=0)**2, axis=1))
    cumulative = np.concatenate(([0], np.cumsum(dists)))
    t = cumulative / cumulative[-1]

    # Estimate average curvature from angles between consecutive segments.
    angles = []
    ...
    # Decide on the number of smoothing points: base of 50, plus extra if curvature is high.
    num_points = int(50 + 100 * avg_curvature)
    num_points = max(num_points, len(points))  # ensure at least as many points as the original
    ...
    # Ensure the endpoints exactly match the original.
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Run the mission:
      ...
      - A new B‑Spline smoothing function is applied to each RRT‑Connect segment,
        inserting additional intermediate waypoints (more if the segment has high curvature)
        so that the drone follows a smooth trajectory.
      - The drone follows the computed (smoothed) path with continuous yaw interpolation.
        For each segment, all intermediate waypoints are sent using non‑blocking
        path facing commands (so the drone does not stop), and then the final waypoint
        is commanded with a blocking call using the desired final yaw.
      - After fully reaching each final waypoint, marker verification is performed.
    ...
    # Process each segment between consecutive ordered points
    for i in range(len(ordered_points) - 1):
        seg_start = ordered_points[i]
        seg_goal = ordered_points[i+1]
        planner_range = fixed_planner_range

        # Compute desired final yaw for the segment.
        ...
        # --- B-Spline Smoothing Applied Here ---
        ...
        current_state = smoothed_path[0]
        N = len(smoothed_path)
        # For intermediate waypoints use non-blocking path facing command
        for j in range(1, N-1):
            point = smoothed_path[j]
            print(f"Continuously moving to intermediate point: {point}")
            # Non-blocking command so the drone does not stop
            success_move = drone_interface.go_to.go_to_point_path_facing_non_blocking(point, speed=SPEED)
            if not success_move:
                print(f"Failed to move to intermediate point: {point}")
                return False, None, None
            time.sleep(SLEEP_TIME)
            current_state = point
        # For the final point in this segment, use a blocking call with desired yaw
        final_point = smoothed_path[-1]
        print(f"Final point of segment: {final_point} with final yaw: {dest_yaw:.2f} rad")
        success_move = drone_interface.go_to.go_to_point_with_yaw(final_point, speed=SPEED, angle=dest_yaw)
        if not success_move:
            print(f"Failed to move to final point: {final_point}")
            return False, None, None
        time.sleep(SLEEP_TIME)
    ...
    for j in range(1, N-1):
        point = smoothed_path[j]
        print(f"Return path moving to intermediate point: {point}")
        success_move = drone_interface.go_to.go_to_point_path_facing_non_blocking(point, speed=SPEED)
        if not success_move:
            print(f"Failed to move to intermediate point on return path: {point}")
            return False, None, None
        time.sleep(SLEEP_TIME)
        current_state = point
    # Final return point with blocking command and appropriate yaw
    final_point = smoothed_path[-1]
    yaw_command = math.atan2(return_start[1]-current_state[1], return_start[0]-current_state[0])
    print(f"Return path final point: {final_point} with yaw: {yaw_command:.2f} rad")
    success_move = drone_interface.go_to.go_to_point_with_yaw(final_point, speed=SPEED, angle=yaw_command)
    if not success_move:
        print(f"Failed to move to final return point: {final_point}")
        return False, None, None
    time.sleep(SLEEP_TIME)
