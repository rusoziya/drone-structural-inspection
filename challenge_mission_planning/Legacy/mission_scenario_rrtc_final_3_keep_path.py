...
# **OMPL imports** (top-level import only; no local re-imports in plan_rrtconnect!)
from ompl import base as ob
from ompl import geometric as og

# For TSP assignment, etc.
from scipy.optimize import linear_sum_assignment  # Hungarian algorithm
from scipy.interpolate import splprep, splev     # B-Spline smoothing

# For ROS Path message conversion
from nav_msgs.msg import Path
from geometry_msgs.msg import PoseStamped

# Import the TrajectoryGenerationModule from the same place as go_to_module, etc.
from as2_python_api.modules.trajectory_generation_module import TrajectoryGenerationModule

# Helper function to convert a list of [x, y, z] points to a ROS Path message
...
# ------------------------
# Helper Functions (Geometry, Collision, etc.)
# ------------------------
...
def plan_rrtconnect(start, goal, obstacles, bounds,
    ...
    """Plan a path from start to goal using OMPL RRTConnect."""
    start_time = time.time()

    # We use the top-level imports 'ob' and 'og' from the top of the file
...
def plan_segment(start, goal, obstacles, bounds,
    ...
    """Plan a single segment, subdividing if needed."""
    ...
    if is_path_collision_free(path, obstacles):
        ...
        if recursion_depth <= 0:
            print("Max recursion reached; using current path even if it might be near obstacles.")
            return path, ptime
...
# Minimal stubs for plotting/logging
...
class SingleNodeDroneMission(DroneInterface):
    def __init__(self, drone_id, use_sim_time=True, verbose=False):
        ...
        # Instantiate the TrajectoryGenerationModule exactly as you do for go_to
        ...
        while time.time() - start_time < timeout:
            ...
            if self.detected_marker_id is not None:
                ...
                if self.detected_marker_id == expected_marker_id:
                    ...
                    print(f"[DEBUG] Marker id {self.detected_marker_id} does not match expected {expected_marker_id}.")
                    self.detected_marker_id = None
...
def verify_marker_and_adjust(drone_interface: SingleNodeDroneMission,
    ...
    if detected_id is None:
        print("Marker verification fallback triggered; proceeding.")
        return True
    print("Marker verification successful.")
    return True

# Drone mission start/end
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    """
    Run the mission:
      - Parse the scenario, plan path segments using RRT‑Connect, apply B‑Spline smoothing,
        and convert each smooth segment to a ROS Path message.
      - Send the entire smoothed trajectory for each segment to the TrajectoryGenerationModule,
        which executes the path continuously.
      - Perform marker verification at the final waypoint.
      - Plan a return path similarly.
    """
    ...
    for i in range(len(ordered_points)-1):
        ...
        print(f"Planning segment from {seg_start} to {seg_goal} with final yaw: {dest_yaw:.2f} rad")
        ...
        # Convert the dense, smooth path to a ROS Path message
        path_msg = create_path_msg(smoothed_path, frame_id="earth")
        print(f"Sending trajectory for segment {i} to trajectory generation module.")
        ...
        if not success_traj:
            print("Trajectory generation failed for segment, aborting mission.")
            return False, None, None
        time.sleep(1.0)
    ...
    path_msg = create_path_msg(smoothed_path, frame_id="earth")
    print("Sending return trajectory to trajectory generation module.")
    success_traj = drone_interface.trajectory_generation.traj_generation_with_yaw(
        path=path_msg,
        speed=SPEED,
        frame_id="earth"
    ...
    if not success_traj:
        print("Trajectory generation failed for return segment.")
        return False, None, None
    time.sleep(1.0)
