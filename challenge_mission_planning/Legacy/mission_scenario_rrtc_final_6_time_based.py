...
# Instead of sending individual waypoint commands, the dense, smooth path is converted to a
# ROS Path message and sent as a single command to the TrajectoryGenerationModule.
# This module handles true continuous trajectory following.
#
# *CHANGE*: Instead of curvature-based sampling, we now sample based on fixed time intervals,
# which aligns the trajectory with a desired control frequency (given a desired speed).
#
# Assumptions:
...
# Control period for time-based sampling (seconds)
CONTROL_DT = 0.1
...
def smooth_path_bspline(path, degree=3, smoothing=0):
    """
    Time-based sampling: Computes the B-Spline and then samples points at fixed time intervals.
    The total time is estimated from the path length divided by SPEED and a control period dt is used.
    """
    ...
    # Estimate total time based on desired speed
    total_time = total_length / SPEED if SPEED > 0 else 1.0
    # Number of points based on fixed control interval
    num_points = int(total_time / CONTROL_DT) + 1

    # Create parameter vector based on cumulative distance (normalized)
    t = cumulative / cumulative[-1]

    # Compute B-spline representation
    tck, u = splprep(points.T, u=t, k=min(degree, len(points)-1), s=smoothing)
    # Use time-based sampling: evenly spaced in time (normalized)
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # B-Spline smoothing using time-based sampling
