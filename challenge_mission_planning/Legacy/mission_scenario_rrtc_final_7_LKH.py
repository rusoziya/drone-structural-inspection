...
# command that orients the drone to the final yaw at the last point. The command is now
# adjusted to only change the yaw in-place (using an in_place flag) without any lateral
# motion to detect the marker.
#
# Assumptions:
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # This command is modified to adjust only the yaw (in_place) without any lateral motion.
        ...
        success_traj_yaw = drone_interface.trajectory_generation.traj_generation_with_yaw(
            ...
            angle=dest_yaw,
            frame_id="earth",
            in_place=True  # Ensure only yaw adjustment without lateral movement
        )
