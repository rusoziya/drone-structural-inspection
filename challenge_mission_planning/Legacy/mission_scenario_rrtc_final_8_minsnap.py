...
#
# A new minimum‐snap (minimum‐jerk) smoothing function is integrated here that smooths the continuous
# path returned by RRT‑Connect. Instead of sending individual waypoint commands, the dense,
...
#
# *CHANGE*: Instead of using B‑Spline smoothing and then performing a final yaw fix,
# we now compute a minimum‐snap smoothed trajectory and then use trajectory generation with yaw
# so that the drone is always oriented toward the desired waypoint yaw.
#
# Assumptions:
...
# For minimum snap smoothing (using quadratic programming)
import cvxpy as cp
...
# TSP, B‑spline (B‑Spline smoothing is no longer used)
...
# New smoothing function: Minimum Snap / Minimum Jerk Trajectory Smoothing via QP
def smooth_path_minimum_snap(path, order=7, samples_per_segment=50):
    """
    Given a list of 3D waypoints, computes a piecewise polynomial trajectory (per dimension)
    of degree 'order' that minimizes the integral of the squared 4th derivative (snap)
    subject to continuity constraints up to the 3rd derivative.
    This is solved using cvxpy.
    """
    import math
    path = np.array(path)  # shape (N, 3)
    M = len(path) - 1     # number of segments
    coeffs = {}  # Will hold coefficients for each dimension 'x','y','z'
    for dim, label in enumerate(['x', 'y', 'z']):
        # Create a list of cvxpy Variables, one for each segment.
        seg_vars = [cp.Variable(order+1) for i in range(M)]
        constraints = []
        cost_terms = []
        # Build Q matrix for the cost: cost = ∫_0^1 (p''''(t))^2 dt.
        # p(t) = sum_{j=0}^{order} a_j t^j, so p''''(t) = sum_{j=4}^{order} a_j * j*(j-1)*(j-2)*(j-3) * t^(j-4)
        Q = np.zeros((order+1, order+1))
        for j in range(4, order+1):
            for k in range(4, order+1):
                Q[j, k] = (j*(j-1)*(j-2)*(j-3) * k*(k-1)*(k-2)*(k-3)) / (j+k-7)
        # Endpoint constraints for each segment.
        for i in range(M):
            # p_i(0) = a[0] = waypoint[i]
            constraints.append(seg_vars[i][0] == path[i, dim])
            # p_i(1) = sum_{j=0}^{order} a[j] = waypoint[i+1]
            constraints.append(cp.sum(seg_vars[i]) == path[i+1, dim])
            # Cost term for segment i.
            cost_terms.append(cp.quad_form(seg_vars[i], Q))
        # Continuity constraints at internal waypoints (for segments i-1 and i) for derivatives 1,2,3.
        for i in range(1, M):
            for d in range(1, 4):
                left_expr = 0
                for j in range(d, order+1):
                    left_expr += seg_vars[i-1][j] * math.factorial(j) / math.factorial(j-d)
                right_expr = math.factorial(d) * seg_vars[i][d]
                constraints.append(left_expr == right_expr)
        # Boundary conditions: initial derivatives (d=1,2,3 at t=0) are zero.
        for d in range(1, 4):
            constraints.append(math.factorial(d) * seg_vars[0][d] == 0)
        # Final boundary conditions: final derivatives (d=1,2,3 at t=1) are zero.
        for d in range(1, 4):
            left_expr = 0
            for j in range(d, order+1):
                left_expr += seg_vars[M-1][j] * math.factorial(j) / math.factorial(j-d)
            constraints.append(left_expr == 0)
        # Solve the QP for this dimension.
        objective = cp.Minimize(cp.sum(cost_terms))
        prob = cp.Problem(objective, constraints)
        prob.solve()
        # Store the optimal coefficients for each segment.
        coeffs[label] = [seg_vars[i].value for i in range(M)]
    # Sample the piecewise polynomial trajectory.
    smoothed_path = []
    M = len(path) - 1
    for i in range(M):
        t_samples = np.linspace(0, 1, samples_per_segment, endpoint=False)
        for t in t_samples:
            x_val = sum(coeffs['x'][i][j] * (t**j) for j in range(order+1))
            y_val = sum(coeffs['y'][i][j] * (t**j) for j in range(order+1))
            z_val = sum(coeffs['z'][i][j] * (t**j) for j in range(order+1))
            smoothed_path.append([x_val, y_val, z_val])
    smoothed_path.append(list(path[-1]))
...
def plot_rrt_tree(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    plt.tight_layout()
    file_path_rrt = os.path.join(OUTPUT_DIR, "RRT_Tree.png")
...
def plot_rrt_tree_3d(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    plt.tight_layout()
    file_path_rrt_3d = os.path.join(OUTPUT_DIR, "RRT_Tree_3D.png")
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Runs the mission:
      - Plans TSP order
      - Plans RRT segments
      - For each segment, applies minimum snap/jerk trajectory smoothing and sends the
        smoothed path using trajectory generation with yaw so that the drone is always
        oriented toward the desired waypoint yaw.
    """
    print("Run mission with RRT‑Connect, TSP (3‑opt), and trajectory generation with yaw.")
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # Apply minimum snap (minimum jerk) smoothing instead of B‑Spline smoothing.
        smoothed_path = smooth_path_minimum_snap(path)
        ...
        # Send the entire segment using trajectory generation with yaw.
