...
#
# *CHANGE*: Instead of following the path in PATH_FACING mode and then doing a final yaw fix,
# we now use trajectory generation with yaw so that the drone is always oriented towards the
# desired waypoint yaw. The final single-point yaw adjustment has been removed.
#
# Assumptions:
...
# 5. RRT‑Connect computes a continuous 3D path.
...
# TSP (Removed B‑Spline smoothing)
    ...
    if obstacles is not None:
        for obs in obstacles:
            ...
            rect = patches.Rectangle(lower_left, dx + 2*SAFETY_MARGIN, dy + 2*SAFETY_MARGIN,
                                     linewidth=1, edgecolor='k', facecolor='gray', alpha=0.6, zorder=1)
            ax2d.add_patch(rect)
    
    # Plot ground truth next (lower zorder)
    gt_x = [p[0] for p in ground_truth]
    gt_y = [p[1] for p in ground_truth]
    ax2d.plot(gt_x, gt_y, 'g--', label="Ground Truth", linewidth=2, zorder=2)
    
    # Plot waypoints
    wp_x = [p[0] for p in waypoints]
    wp_y = [p[1] for p in waypoints]
    ax2d.plot(wp_x, wp_y, 'ro-', label="Waypoints", linewidth=2, zorder=3)
    
    # Finally, plot drone paths on top
    for idx, path in enumerate(planned_paths):
        xs = [p[0] for p in path]
        ys = [p[1] for p in path]
        if idx == 0:
            ax2d.plot(xs, ys, label="Drone Path", color='black', linewidth=2, zorder=4)
        else:
            ax2d.plot(xs, ys, color='black', linewidth=2, zorder=4)
    
    ax2d.set_xlabel("X (m)", fontsize=14)
    ax2d.set_ylabel("Y (m)", fontsize=14)
    ax2d.set_title("2D Trajectory of Planned Paths", fontsize=16)
    ax2d.legend(fontsize=6)
...
def plot_rrt_tree(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    if obstacles is not None:
        for obs in obstacles:
            ...
            lower_left = (ox - dx/2 - SAFETY_MARGIN, oy - dy/2 - SAFETY_MARGIN)
            rect = patches.Rectangle(lower_left, dx + 2*SAFETY_MARGIN, dy + 2*SAFETY_MARGIN,
                                     linewidth=1, edgecolor='k', facecolor='gray', alpha=0.45, zorder=2)
            ax.add_patch(rect)
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Runs the mission:
      - Plans TSP order
      - Plans RRT segments
      - Sends path with yaw such that the drone is always oriented towards the desired waypoint yaw.
    """
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # Compute the desired final yaw for this segment
        ...
        # Removed B‑Spline smoothing; using original path instead
        smoothed_path = path
        ...
        # Use trajectory generation with yaw for the segment
        path_msg = create_path_msg(smoothed_path, frame_id="earth")
        print(f"Following path with yaw (final yaw: {dest_yaw:.2f} rad).")
        ...
        # Marker verification remains unchanged
    ...
    # Compute desired yaw for return path (from last waypoint to start)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Single drone mission with RRT‑Connect, TSP (3‑opt), obstacle avoidance, and trajectory generation with yaw enabled'
    )
