...
def plot_paths(planned_paths, waypoints, obstacles=None, markers_list=None):
    """
    Plot the 2D and 3D trajectory of the planned drone paths and waypoints.
    Ground truth is omitted.
    """
    # 2D Trajectory Plot
    ...
    if obstacles is not None:
        for obs in obstacles:
            ...
            ax3d.add_collection3d(Poly3DCollection([cuboid], facecolors='gray', alpha=0.3, edgecolor='k'))
    ax3d.set_xlabel("X (m)", fontsize=14)
    ax3d.set_ylabel("Y (m)", fontsize=14)
    ax3d.set_zlabel("Z (m)", fontsize=14)
