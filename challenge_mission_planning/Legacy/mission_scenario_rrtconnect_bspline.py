...
from scipy.interpolate import CubicHermiteSpline  # Cubic interpolation for smoothing
...
def is_path_collision_free(path, obstacles, resolution=COLLISION_CHECK_RESOLUTION):
    ...
    for i in range(len(path)-1):
        ...
        for j in range(steps + 1):
            ...
            if not is_state_valid_cuboids(interp_pt, obstacles):
                return False
    return True
...
# --------------------------
# Cubic Polynomial Trajectory Smoothing
# --------------------------
def cubic_trajectory_smoothing(path, dest_yaw, num_points=4):
    """
    Given a list of 3D points (path) and a desired final yaw (dest_yaw),
    compute a smoothed trajectory using cubic Hermite spline interpolation.
    Returns a tuple (smoothed_points, smoothed_yaws) where:
      - smoothed_points is a list of [x,y,z] points
      - smoothed_yaws is a list of yaw angles (in radians), with the final yaw forced to dest_yaw.
    """
    path = np.array(path)
    n_points = len(path)
    # Compute cumulative distances to parameterize the path
    t = [0]
    for i in range(1, n_points):
        t.append(t[-1] + np.linalg.norm(path[i] - path[i-1]))
    t = np.array(t)
    # Estimate derivatives (tangents) at each waypoint
    m = np.zeros_like(path)
    m[0] = (path[1] - path[0]) / (t[1] - t[0])
    m[-1] = (path[-1] - path[-2]) / (t[-1] - t[-2])
    for i in range(1, n_points-1):
        m[i] = (path[i+1] - path[i-1]) / (t[i+1] - t[i-1])
    # Create a cubic Hermite spline for the trajectory
    spline = CubicHermiteSpline(t, path, m)
    t_new = np.linspace(t[0], t[-1], num_points)
    smoothed_points = spline(t_new)
    # Compute derivative of the spline for yaw estimation
    spline_deriv = spline.derivative()
    v = spline_deriv(t_new)
    smoothed_yaws = []
    for i in range(len(t_new)):
        dx, dy = v[i, 0], v[i, 1]
        if np.linalg.norm([dx, dy]) < 1e-3:
            # If near-zero velocity, reuse previous yaw or set to dest_yaw if first point
            yaw = smoothed_yaws[-1] if i > 0 else dest_yaw
        else:
            yaw = math.atan2(dy, dx)
        smoothed_yaws.append(yaw)
    # Force the final yaw to be the desired destination yaw
    smoothed_yaws[-1] = dest_yaw
    return smoothed_points.tolist(), smoothed_yaws
...
def plan_rrtconnect(start, goal, obstacles, bounds, planner_range=PLANNER_RANGE_SMALL, planning_time_limit=PLANNING_TIME_LIMIT):
    ...
    if solved:
        ...
        path_simplifier.simplifyMax(path_obj)
        path_states = []
...
def compute_path_smoothness(path):
    ...
    Smoothness is estimated as the sum of absolute changes in travel direction (in radians).
    """
    if len(path) < 3:
        return 0.0
    smoothness = 0.0
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Run the mission:
      ...
      - The drone follows a smoothed trajectory generated via a cubic polynomial (Hermite spline).
      - The yaw is computed from the spline’s derivative and the final yaw is forced to the desired orientation.
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # --- Cubic Polynomial Trajectory Smoothing ---
        smoothed_points, smoothed_yaws = cubic_trajectory_smoothing(path, dest_yaw, num_points=4)
        for pt, yaw in zip(smoothed_points, smoothed_yaws):
            print(f"Moving to point: {pt} with yaw: {yaw:.2f} rad")
            success_move = drone_interface.go_to.go_to_point_with_yaw(pt, angle=yaw, speed=SPEED)
            if not success_move:
                print(f"Failed to move to point: {pt}")
                return False, None, None
    ...
    smoothed_points, smoothed_yaws = cubic_trajectory_smoothing(path, 
                                            math.atan2(return_start[1]-path[-1][1], return_start[0]-path[-1][0]),
                                            num_points=4)
    for pt, yaw in zip(smoothed_points, smoothed_yaws):
        print(f"Return path moving to point: {pt} with yaw: {yaw:.2f} rad")
        success_move = drone_interface.go_to.go_to_point_with_yaw(pt, angle=yaw, speed=SPEED)
        if not success_move:
            print(f"Failed to move to point: {pt} on return path")
            return False, None, None
        time.sleep(SLEEP_TIME)
