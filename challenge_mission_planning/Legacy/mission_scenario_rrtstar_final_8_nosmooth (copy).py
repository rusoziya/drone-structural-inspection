...
#
# *CHANGE*: Instead of using RRT‑Connect, we now use OMPL’s RRT* for motion planning.
#
# Assumptions:
...
# 5. RRT* computes a continuous 3D path.
...
PLANNING_TIME_LIMIT = 1        # Planning time limit per segment (s)
...
def plan_segment(start, goal, obstacles, bounds,
    ...
    # Use RRT* for planning instead of RRT‑Connect
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Single drone mission with RRT*, TSP (3‑opt), obstacle avoidance, and trajectory generation with yaw enabled'
    )
    ...
    if plot_data is not None and metrics is not None:
        ...
        # The following RRT* tree plotting functions are commented out:
