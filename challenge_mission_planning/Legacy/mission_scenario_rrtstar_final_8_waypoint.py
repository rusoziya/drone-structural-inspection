...
#
# 2. Viewpoint poses are specified under "viewpoint_poses" in the scenario YAML,GPG
...
def plot_rrt_tree(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    """
    Plot the 2D projection of the RRT* tree along with waypoints, obstacles,
...
def plot_rrt_tree_3d(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    """
    Plot the 3D version of the RRT* tree along with waypoints, obstacles,
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Run the mission:
      ...
      - For each segment between ordered points, plan a collision-free path using 
        OMPL RRT* (with recursive subdivision if necessary).
    ...
    """
    print("Run mission with RRT* planning, TSP (3‑opt local search) optimization, continuous commands, obstacle avoidance, and marker verification")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Single drone mission with RRT* planning, TSP (3‑opt local search) optimization, obstacle avoidance, and marker verification enabled'
    )
    ...
    # Mission termination: call external stop script before exit.
    os.system("./stop.bash")
    print("Clean exit")
    exit(0)
