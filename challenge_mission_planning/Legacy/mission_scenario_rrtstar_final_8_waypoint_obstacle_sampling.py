...
#
# 5. The OMPL-based RRT* algorithm is used to compute a continuous 3D path,
...
PLANNING_TIME_LIMIT = 1       # Time limit for planning each segment (in seconds) 
PLANNER_RANGE_SMALL = 1.0          # Fixed planner range used for all segments 

# ------------------------
# Custom Sampling Parameters (for hybrid sampling)
# ------------------------
BIAS_PROBABILITY = 0.3      # Probability to sample near obstacles
BIAS_RADIUS = 2.0           # Bias sampling radius (meters) around obstacles
...
# ------------------------
# Custom State Sampler (Modified Sampling Distribution)
# ------------------------
# Inherit from ob.ValidStateSampler (as required) and get the default valid sampler.
class CustomStateSampler(ob.ValidStateSampler):
    def __init__(self, space, obstacles, bias_probability, bias_radius):
        super(CustomStateSampler, self).__init__(space)
        # Allocate the space's default valid state sampler.
    ...
    def sampleUniform(self, state):
        if random.random() < self.bias_probability and self.obstacles:
            # Choose a random obstacle from the list
            obs = random.choice(self.obstacles)
            center = np.array([obs["x"], obs["y"], obs["z"]])
            # Sample uniformly in a sphere of radius = bias_radius around the obstacle center
            r = self.bias_radius * (random.random() ** (1/3))
            ...
            # Check that the sample is within bounds; if not, fallback to uniform sampling
            bounds = self.getSpace().getBounds()
            if (sample[0] < bounds.low[0] or sample[0] > bounds.high[0] or
                sample[1] < bounds.low[1] or sample[1] > bounds.high[1] or
                sample[2] < bounds.low[2] or sample[2] > bounds.high[2]):
...
# --------------------------
# RRT* Path Planning (Modified Sampling)
# --------------------------
def plan_rrtstar(start, goal, obstacles, bounds, planner_range=PLANNER_RANGE_SMALL, planning_time_limit=PLANNING_TIME_LIMIT):
    start_time = time.time()
    # First, check if the direct straight-line path is collision-free.
    if is_path_collision_free([start, goal], obstacles):
        planning_time = time.time() - start_time
        return [start, goal], planning_time

    # Set up the state space and bounds.
    ...
    # Define a sampler allocator function that returns an instance of our custom valid state sampler.
    def my_sampler_allocator_wrapper(si_ptr, obstacles_param, bias_prob, bias_rad):
        sampler = CustomStateSampler(si_ptr.getStateSpace(), obstacles_param, bias_prob, bias_rad)
        return ob.ValidStateSamplerPtr(sampler)
    
    # Set the valid state sampler allocator.
    si.setValidStateSamplerAllocator(
        lambda si_ptr: my_sampler_allocator_wrapper(
            si_ptr, obstacles_param=obstacles, bias_prob=BIAS_PROBABILITY, bias_rad=BIAS_RADIUS
        )
    )    # ----------------------------------------
...
def plan_segment(start, goal, obstacles, bounds, planner_range, planning_time_limit, recursion_depth=MAX_RECURSION_DEPTH):
    path, ptime = plan_rrtstar(start, goal, obstacles, bounds, planner_range, planning_time_limit)
    if path is None:
        return None, ptime
...
def plot_rrt_tree(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    plotted_rrt_nodes_legend = False
...
def plot_rrt_tree_3d(rrt_tree_data, waypoints=None, obstacles=None, planned_paths=None):
    ...
    if obstacles is not None:
        for obs in obstacles:
            ...
            cuboid = [
                [x, y, oz - dz/2 - SAFETY_MARGIN],
                [x+dx+2*SAFETY_MARGIN, y, oz - dz/2 - SAFETY_MARGIN],
                [x+dx+2*SAFETY_MARGIN, y+dy+2*SAFETY_MARGIN, oz - dz/2 - SAFETY_MARGIN],
                [x, y+dy+2*SAFETY_MARGIN, oz - dz/2 - SAFETY_MARGIN],
                [x, y, oz + dz/2 + SAFETY_MARGIN],
                [x+dx+2*SAFETY_MARGIN, y, oz + dz/2 + SAFETY_MARGIN],
                [x+dx+2*SAFETY_MARGIN, y+dy+2*SAFETY_MARGIN, oz + dz/2 + SAFETY_MARGIN],
                [x, y+dy+2*SAFETY_MARGIN, oz + dz/2 + SAFETY_MARGIN]
...
def plot_paths(planned_paths, waypoints, obstacles=None, markers_list=None):
    ...
    def draw_obstacle_3d_wireframe(ax3d, ox, oy, oz, dx, dy, dz):
        rx1 = ox - dx/2; rx2 = ox + dx/2
        ry1 = oy - dy/2; ry2 = oy + dy/2
        rz1 = oz - dz/2; rz2 = oz + dz/2
        mx1 = rx1 - SAFETY_MARGIN; mx2 = rx2 + SAFETY_MARGIN
        my1 = ry1 - SAFETY_MARGIN; my2 = ry2 + SAFETY_MARGIN
        mz1 = rz1 - SAFETY_MARGIN; mz2 = rz2 + SAFETY_MARGIN
        ...
        ax3d.plot([mx1, mx1], [my2, my2], [mz2, mz2], 'k-', linewidth=0.5, alpha=0.2, label='_nolegend_')
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Run the mission:
      ...
      - For each segment between ordered points, plan a collision-free path using 
        OMPL RRT* (with recursive subdivision if necessary and with direct-path shortcutting).
    ...
    """
    print("Run mission with RRT* planning (with direct-path shortcuts and biased sampling), TSP (3‑opt local search) optimization, continuous commands, obstacle avoidance, and marker verification")
    ...
    total_ground_truth_path_length = sum(compute_euclidean_distance(ordered_points[i], ordered_points[i+1]) 
                                          for i in range(len(ordered_points)-1))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Single drone mission with RRT* planning (with direct-path shortcuts and biased sampling), TSP (3‑opt local search) optimization, obstacle avoidance, and marker verification enabled'
    )
    ...
    if plot_data is not None:
        ...
        plot_costs(plot_data["segment_planning_times"],
