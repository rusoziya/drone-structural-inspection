...
#
# *CHANGE*: Instead of always invoking the RRT* planner for each segment, we now first check if
# the direct (straight‑line) path is collision‑free. If it is, we command the drone to fly directly
# using a single go‑to command with continuous yaw interpolation (using drone_interface.go_to.go_to_point_with_yaw).
#
# Assumptions:
...
SAFETY_MARGIN = 0.5        # Additional safety margin (m) around obstacles
...
PLANNING_TIME_LIMIT = 2.0        # Planning time limit per segment (s)
...
# Removed: from scipy.interpolate import splprep, splev
...
# Removed B‑Spline smoothing functionality; the function now returns the original path.
def smooth_path_bspline(path, degree=3, smoothing=0):
    # B‑Spline smoothing has been removed. Returning original path.
    return path
...
class SingleNodeDroneMission(DroneInterface):
    ...
    def image_callback(self, msg):
        ...
        # If you literally want 1/4 of each dimension (resulting in 1/16 area),
...
def drone_run(drone_interface: SingleNodeDroneMission, scenario: dict):
    ...
    Runs the mission:
      - Plans TSP order
      - Plans RRT* segments or uses a direct go‑to command if the straight‑line path is free of obstacles.
      - Sends each segment using trajectory generation with yaw so that the drone is
        always oriented toward the desired waypoint yaw (or a direct go‑to command for collision‑free segments).
    """
    ...
    for i in range(len(ordered_points) - 1):
        ...
        # ### CHANGE: Check if the direct path is collision-free.
        if is_path_collision_free([seg_start, seg_goal], obstacles):
            print("Direct path is collision-free. Executing direct go-to command.")
            success_direct = drone_interface.go_to.go_to_point_with_yaw(point=seg_goal, angle=dest_yaw, speed=SPEED)
            if not success_direct:
                print("Direct go-to command failed, aborting mission.")
                return False, None, None
            time.sleep(1.0)
            direct_path = [seg_start, seg_goal]
            planned_paths.append(direct_path)
            seg_length = compute_euclidean_distance(seg_start, seg_goal)
            total_path_length += seg_length
            segment_planning_times.append(0.0)
            segment_lengths.append(seg_length)
            current_point = seg_goal
        else:
            # Fall back to RRT* planning
            ...
            # B‑Spline smoothing removed; using original path instead.
            smoothed_path = path
        ...
        # Marker verification remains the same
    ...
    # ### CHANGE: Check if the return path is collision-free.
    if is_path_collision_free([last_waypoint, return_start], obstacles):
        print("Direct return path is collision-free. Executing direct go-to command.")
        success_direct = drone_interface.go_to.go_to_point_with_yaw(point=return_start, angle=dest_yaw_return, speed=SPEED)
        if not success_direct:
            print("Direct go-to command for return path failed, aborting mission.")
            return False, None, None
        time.sleep(1.0)
        direct_path = [last_waypoint, return_start]
        planned_paths.append(direct_path)
        seg_length = compute_euclidean_distance(last_waypoint, return_start)
        total_path_length += seg_length
        segment_planning_times.append(0.0)
        segment_lengths.append(seg_length)
    else:
